...
export class AutoCompleteSelectorComponent implements OnInit, ControlValueAccessor, OnChanges {
    ...
    ngOnInit() {
        ...
        if (this.initList) {
            this.itemListSubscription = this.tableService.find<Record<string, unknown>>(this.collectionName, this.filters, this.populate, this.showDrafts,'asc', 'createdAt', 0, -1, this.activeLang)
    ...
    writeValue(controls?: any): void {
        if (controls) {
            ...
            this.itemListSubscription?.unsubscribe()
            this.itemListSubscription = this.tableService.find(this.collectionName, [filter], this.populate, this.showDrafts,'asc', 'createdAt', 0, -1, this.activeLang)
    ...
    search<T>(searchQuery: string): Observable<CollectionResponse<T>> {
        ...
        return this.tableService.find<T>(this.collectionName, [...this.filters, filter], this.populate, this.showDrafts,'asc', 'createdAt', 0, -1, this.activeLang)
    }
}
